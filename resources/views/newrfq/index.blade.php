...
$(document).on('keyup', '#MultipleCountry td input', function() {
    ...
    $('#MultipleCountry tbody td:has(input[attr="total"])').each(function(){
        ...
        let cpi = parseFloat($(this).prev().find('input').val()); // Use parseFloat for decimals
        let sample = parseFloat($(this).prev().prev().find('input').val()); // Use parseFloat for decimals
    ...
    $('#MultipleCountry tbody td:has(input[name="multiple_total_cost[]"])').each(function() {
        ...
        $(this).find('input').val(sum.toFixed(2)); // Ensure sum is also in two decimal places
